...
/**
 * lunar calendar  and solar calendar convert
 * @author yuehaijun
 */
...
public class LunarCalendar {
	...
	public static void main(String[] args) {
		String y = LunarCalendar.lundarToSolar(2008, 1,1);
		y=LunarCalendar.solarToLundar(2007, 2, 23);
	...
	// from 1901 to 2100 of the lunar LunarCalendar,
	// The lunar LunarCalendar can only be 29 or 30 days every month, express with
	...
	// Array iLunarLeapMonthTable preserves the lunar LunarCalendar leap month from
	// 1901 to 2050,
	...
	// in New Year of solar LunarCalendar and lunar LunarCalendar from 1901 to 2050;
	...
	// The days in the month of solar LunarCalendar
	...
	// Days in this year of lunar LunarCalendar
	...
	// The solar LunarCalendar is turned into the lunar LunarCalendar
	public static String solarToLundar(int iYear, int iMonth, int iDay) {
	...
	// The lunar LunarCalendar is turned into the Solar LunarCalendar
	public static String lundarToSolar(int iYear, int iMonth, int iDay) {
...
class SolarDate extends MyDate {
	...
	public LunarDate toLunarDate() {
		...
		LunarDate ld;
		iDate = Integer.parseInt(LunarCalendar.solarToLundar(this.iYear,
...
class LunarDate extends MyDate {
	...
	private String sChineseNum[] = { "��", "һ", "��", "��", "��", "��", "��", "��",
			"��", "��", "ʮ" };

	private static int checkMonth(int iYear, int iMonth) {
		if ((iMonth > 12) && (iMonth == LunarCalendar.iGetLLeapMonth(iYear) + 12)) {
			return iMonth;
		} else if (iMonth > 12) {
			System.out.println("Month out of range, I think you want 12 :)");
			return 12;
	...
	private static int checkDay(int iYear, int iMonth, int iDay) {
		int iMonthDays = LunarCalendar.iGetLMonthDays(iYear, iMonth);
	...
	public String toString() {
		String sLunarCalendar = "ũ��";

		sLunarCalendar += sChineseNum[iYear / 1000]
		...
		if (iMonth == 12)
			...
			sLunarCalendar += sChineseNum[iMonth] + "��";
		if (iDay > 29)
			sLunarCalendar += "��ʮ";
		else if (iDay > 20)
			sLunarCalendar += "��ʮ" + sChineseNum[iDay % 20];
		else if (iDay == 20)
			sLunarCalendar += "��ʮ";
		else if (iDay > 10)
			sLunarCalendar += "ʮ" + sChineseNum[iDay % 10];
		else
			sLunarCalendar += "��" + sChineseNum[iDay];

		return sLunarCalendar;
	}

	public CnWeek toWeek() {
		int iOffsetDays = 0;
		for (int i = 1901; i < iYear; i++)
			iOffsetDays += LunarCalendar.iGetLYearDays(i);

		iOffsetDays += LunarCalendar.iGetLNewYearOffsetDays(iYear, iMonth, iDay);
	...
	public SolarDate toSolarDate() {
		int iYear, iMonth, iDay, iDate;
		SolarDate sd;
		iDate = Integer.parseInt(LunarCalendar.lundarToSolar(this.iYear,
				this.iMonth, this.iDay));
		iYear = iDate / 10000;
		iMonth = iDate % 10000 / 100;
		iDay = iDate % 100;
...
class ChineseEra {
	...
	String[] sHeavenlyStems = new String[] { "��", "��", "��", "��", "��", "��", "��",
			"��", "��", "��" };

	String[] sEarthlyBranches = new String[] { "��", "��", "��", "î", "��", "��",
			"��", "δ", "��", "��", "��", "��" };
